...
# ['manager','data',self.name,correct,points_data['questionPoints'],data_content['totalScore'],streak_data['streakLevel'],data_content['rank']]
...
class bot:
                ...
                for i, x in enum(response):
                    if x['channel'] == "/service/player":
                        ...
                        if data['id'] == 3:
                            ...
                            game_type = data_content['gameBlockType']
                            self.answer_question([foo, game_type, question_num, len(answers)])
                        elif data['id'] == 8:
                            data_content = json.loads(data['content'])
                            points_data = data_content['points_data']
                            streak_data = points_data['answerStreakPoints']
                            try:
                                correct = data_content['isCorrect']
                            except:
                                correct = True
                            self.queue.put(['manager', 'data', self.name, correct, points_data['questionPoints'], data_content['totalScore'], streak_data['streakLevel'], data_content['rank']])
                        elif data['id'] == 4:
                            break
                    else:
                        continue
    ...
    def answer_question(self, get):
        foo, game_type, index, amount = get
        if game_type == 'multiple_select_quiz':
            choice = [*range(foo + 1)]
            ...
            choice = [*range(4)]
            random.shuffle(choice)
        elif game_type == 'open_ended':
            choice = randomString(random.randint(0, 20))
        else:
            choice = random.randint(0, foo)
            game_type = 'quiz'
        self.queue.put(['manager', 'answer', self.name, choice, game_type, index, amount])
        wait()
        self.s.post(self.url, data=answer_payload(self.pin, self.clientId, self.subId, choice, game_type))
